...
@pytest.mark.asyncio
async def test_workspace_initialization_removes_legacy_thread_links(tmp_path: Path, monkeypatch) -> None:
    monkeypatch.setattr(workspace_paths.conf, "save_dir", str(tmp_path))
    user = _user()
    thread_id = "thread-deleted"
    _make_thread_files(tmp_path, thread_id)
    agents_dir = tmp_path / "threads" / "shared" / "user-1" / "workspace" / "agents"
    legacy_thread_dir = agents_dir / thread_id
    legacy_thread_dir.mkdir(parents=True)
    (legacy_thread_dir / "uploads").symlink_to(
        tmp_path / "threads" / thread_id / "user-data" / "uploads", target_is_directory=True
    )
    (legacy_thread_dir / "outputs").symlink_to(
        tmp_path / "threads" / thread_id / "user-data" / "outputs", target_is_directory=True
    )
    assert (agents_dir / thread_id / "uploads").is_symlink()

    svc._workspace_root(user)

    assert not (agents_dir / thread_id).exists()


@pytest.mark.asyncio
async def test_legacy_cleanup_keeps_manual_thread_directory_content(tmp_path: Path, monkeypatch) -> None:
    monkeypatch.setattr(workspace_paths.conf, "save_dir", str(tmp_path))
    user = _user()
    ...
    agents_dir = tmp_path / "threads" / "shared" / "user-1" / "workspace" / "agents"
    agents_dir.mkdir(parents=True)
    (agents_dir / thread_id).mkdir()
    (agents_dir / thread_id / "uploads").write_text("用户手动文件", encoding="utf-8")

    svc._workspace_root(user)

    assert (agents_dir / thread_id / "uploads").read_text(encoding="utf-8") == "用户手动文件"
