...
- 远程 Skill 来源策略迁移到 PostgreSQL 系统配置，默认仅允许 GitHub 与 ModelScope；`owner/repo` 和 HTTPS URL 在调用 Skills CLI 前统一校验，空白名单会关闭远程安装；同时将不可信 Skills CLI 隔离到一次性 Sandbox，并在复制及导出链路拒绝符号链接，修复 #895 的宿主文件读取风险。
