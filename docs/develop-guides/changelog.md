...
- 远程 Skill 来源策略迁移到 PostgreSQL，并在「基本设置」提供白名单配置；默认允许 GitHub 与 ModelScope，空列表会关闭远程安装。远程 Skill CLI 改用无环境凭据的一次性 Sandbox，Kubernetes 不挂载 ServiceAccount token，数据回流拒绝符号链接并限制目录规模，修复 #895 并缩小 #855 的风险面。
